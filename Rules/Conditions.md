...
You use all your actions to attack, though the GM can have you use other actions to facilitate attack, such as draw a weapon, move so target is in reach, and so forth. Your targets are determined randomly by the GM. If you have no other viable targets, you target yourself, automatically hitting but not scoring a critical hit. If it's impossible for you to attack, you babble incoherently, wasting your actions.  
...
# Fatigued
You're tired and can't summon much energy. Fatigued always includes a value. You take a status penalty equal to the condition value to AC and saving throws. You recover from fatigue after a full night's rest.
...
# Paralyzed
You're frozen in place. You have the [[#Off-Guard]] condition and can't act except to use actions that require only your mind (as determined by the GM). Your senses still function, but only in the areas you can perceive without moving, so you can't *Seek*.
...
# Temporary Info
**Clumsy X**  
Automatically reduces DEX-based values by X. That means AC, Reflex, DEX weapon attacks, Acrobatics, Stealth, Thievery, and DEX-derived Power Attacks.

**Enfeebled X**  
Automatically reduces STR-based values by X: STR melee attacks, STR damage, Athletics, and STR-derived Power Attacks.

**Frightened X**  
Automatically reduces applicable checks and defenses by X and decreases by 1 when **Next Round** is clicked. This is the one where our Round button approximation differs slightly from the source text's "end of each of your turns," but it makes sense for the current combat tracker.

**Off-Guard**  
AC −2.

**Prone**  
Automatically includes Off-Guard's AC −2 and applies −2 to attacks. The movement restrictions remain displayed as rules text rather than something the sheet needs to enforce.

**Stupefied X**  
Automatically affects INT-, WIS-, and CHA-derived calculations, including Will and Powers using an Essence associated with one of those attributes.

**Fatigued X**  
Automatically modifies AC, Fortitude, Reflex, and Will by −X based on the current definition.

The rest can initially be **tracked conditions with reminders** without pretending the sheet understands mechanics that Confluence hasn't finalized.